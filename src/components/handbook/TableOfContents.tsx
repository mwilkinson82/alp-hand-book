...
const tocData: TocSection[] = [
  {
    ...
    items: [
      ...
      {
        id: 'foreword',
        title: "Foreword / Author's Note",
        badge: 'Updated for V2',
      },
      {
        id: 'how-to-use',
        ...
        {tocData.map((section, sectionIndex) => (
          <div key={sectionIndex} className="mb-2">
            ...
            {section.items.map((item) => (
              <div key={item.id} className="toc-item flex items-baseline gap-6" onClick={() => handleClick(item.id)}>
                <span className="font-sans text-xs uppercase tracking-[0.25em] opacity-50 w-10 shrink-0">{item.chapter ?? ''}</span>
