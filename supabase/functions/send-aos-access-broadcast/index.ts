...
import {
  AOS_BROADCAST_HTML,
  AOS_BROADCAST_SUBJECT,
  AOS_BROADCAST_PREVIEW,
...
const BROADCAST_ID = "aos-access-launch";
...
serve(async (req) => {
  ...
  try {
    ...
    const html = AOS_BROADCAST_HTML.replace(
    ...
    if (mode === "html") {
      return new Response(JSON.stringify({ html, subject: AOS_BROADCAST_SUBJECT }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
    }
    ...
    console.error("send-aos-access-broadcast error:", msg);
