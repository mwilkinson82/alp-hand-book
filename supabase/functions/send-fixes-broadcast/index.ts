...
import {
  FIXES_BROADCAST_HTML,
  FIXES_BROADCAST_SUBJECT,
  FIXES_BROADCAST_PREVIEW,
...
const BROADCAST_ID = "reader-fixes-2026-06";
...
async function sendEmail(to: string, html: string): Promise<{ ok: boolean; error?: string }> {
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM_ADDRESS,
      to: [to],
      subject: FIXES_BROADCAST_SUBJECT,
      html,
      headers: {
...
serve(async (req) => {
  ...
  try {
    ...
    const html = FIXES_BROADCAST_HTML.replace(
    ...
    if (mode === "preview") {
      return new Response(
        JSON.stringify({
          subject: FIXES_BROADCAST_SUBJECT,
          preview: FIXES_BROADCAST_PREVIEW,
    ...
    if (mode === "html") {
      return new Response(JSON.stringify({ html, subject: FIXES_BROADCAST_SUBJECT }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
    }
    ...
    console.error("send-fixes-broadcast error:", msg);
